...
int __original_main(int _argc_ignored, char **_argv_ignored)
{
  ...
  mpz_t **a;
  a = (mpz_t **)malloc(sizeof(mpz_t *) * (bS));
  for (int _picco_i = 0; _picco_i < bS; _picco_i++)
  {
    a[_picco_i] = (mpz_t *)malloc(sizeof(mpz_t) * (S));
    for (int j = 0; j < S; ++j)
    {
      mpz_init(a[_picco_i][j]);
    }
  }

  mpz_t **b;
  b = (mpz_t **)malloc(sizeof(mpz_t *) * (bS));
  for (int _picco_i = 0; _picco_i < bS; _picco_i++)
  {
    b[_picco_i] = (mpz_t *)malloc(sizeof(mpz_t) * (S));
    for (int j = 0; j < S; ++j)
    {
      mpz_init(b[_picco_i][j]);
    }
  }

  mpz_t **c;
  c = (mpz_t **)malloc(sizeof(mpz_t *) * (bS));
  for (int _picco_i = 0; _picco_i < bS; _picco_i++)
  {
    c[_picco_i] = (mpz_t *)malloc(sizeof(mpz_t) * (S + 1));
    for (int j = 0; j < S + 1; ++j)
    {
      mpz_init(c[_picco_i][j]);
    }
  }

  mpz_t *res;
  res = (mpz_t *)malloc(sizeof(mpz_t) * (bS));
  for (int _picco_i = 0; _picco_i < bS; _picco_i++)
    mpz_init(res[_picco_i]);
  ...
  for (; i < rep;)
  {
    __s->smc_bitcomp(a, b, res, S, bS, -1);
    i++;
  }
  gettimeofday(&end, NULL);
  timerp1 = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;
...
int main(int argc, char **argv)
{
  ...
  __s = new SMC_Utils(atoi(argv[1]), argv[2], argv[3], atoi(argv[4]), atoi(argv[5]), IO_files, 3, 1, 2, "2", 1);
