...
//	ss->checkSeed();
	//malicious
	MSmc = new MaliciousSMC(*nodeNet, ss, numOfPeers, threshold);
	setCoef(); 
	Mul = new Mult(*nodeNet, id, ss);
	BitA = new BitAdd(*nodeNet, polynomials, id, ss, coef, MSmc); 
	BitC = new BitComp(*nodeNet, polynomials, id, ss, coef, MSmc); 
	BitE = new BitEq(*nodeNet, polynomials, id, ss, coef, MSmc); 
...
void SMC_Utils::smc_communication_summary() {
	nNet.getSummary();
}
...
void SMC_Utils::smc_bitcomp(mpz_t** a, mpz_t** b, mpz_t* s, int size, int batch_size, int threadID){
	BitC->doOperation(a, b, s, size, batch_size, threadID);
}
...
void SMC_Utils::smc_bitcomp_mal(mpz_t*** a, mpz_t*** b, mpz_t** s, int size, int batch_size, int threadID){
	BitC->doOperation_mal(a, b, s, size, batch_size, threadID);
}
...
void SMC_Utils::smc_init_mal(){
  ...
  PRand->generateRandValue(pid, field, 1, r_temp); //TODO: find a good way to generate r
...
void SMC_Utils::smc_input_Onlymul(mpz_t** a, int input_num, int size, int alen, std::string type, int threadID){
  mpz_t* rArray;
  mpz_t* multArray;
  mpz_t* multArray1;
  rArray = (mpz_t*)malloc(sizeof(mpz_t) * (size * input_num));
  multArray = (mpz_t*)malloc(sizeof(mpz_t) * (size * input_num));
  multArray1 = (mpz_t*)malloc(sizeof(mpz_t) * (size * input_num));
  for(int i = 0; i < size * input_num; ++i) {
    mpz_init_set(rArray[i], MSmc->r);
    mpz_init(multArray1[i]);
  }
  for(int i = 0; i < input_num; ++i){
    for(int j = 0; j < size; ++j){
      mpz_init_set(multArray[i*size + j], a[0][j]);
    }
  }
  Mul->doOperation(multArray1, multArray, rArray, size * input_num, threadID);
  MSmc->pushBuffer(multArray, multArray1, size * input_num);
  for(int i = 0; i < input_num; ++i){
    for(int j = 0; j < size; ++j){
      mpz_set(a[1][j], multArray1[j]);
    }
  }
  for (int i = 0; i < size * input_num; ++i){
    mpz_clear(rArray[i]);
    mpz_clear(multArray[i]);
    mpz_clear(multArray1[i]);
  }
  free(rArray);
  free(multArray);
  free(multArray1);
...
void SMC_Utils::smc_mult_mal(mpz_t** a, mpz_t** b, int alen, int blen, mpz_t** result, int resultlen, int size, std::string type, int threadID)
{
	...
	for (int _picco_i = 0; _picco_i < curSize; _picco_i++){
		mpz_init(a_temp[_picco_i]);
		mpz_set(a_temp[_picco_i], a[_picco_i/size][_picco_i%size]);
	}
	b_temp = (mpz_t*)malloc(sizeof(mpz_t) * (curSize));
	for (int _picco_i = 0; _picco_i < curSize; _picco_i++){
		mpz_init(b_temp[_picco_i]);
		mpz_set(b_temp[_picco_i], b[0][_picco_i%size]);
	}
	...
	//clean
	for (int _picco_i = 0; _picco_i < curSize; _picco_i++)
		mpz_clear(a_temp[_picco_i]);
	free(a_temp);
	for (int _picco_i = 0; _picco_i < curSize; _picco_i++)
		mpz_clear(b_temp[_picco_i]);
	free(b_temp);
}
...
void SMC_Utils::smc_leq_mal(mpz_t** a, mpz_t** b, int alen, int blen, mpz_t** result, int resultlen, int size, std::string type, int threadID)
{
	  mpz_t** sub = (mpz_t**)malloc(sizeof(mpz_t*) * 2);
	  for(int k = 0; k < 2; k++){
	  ...
	  /*
	  //open result:
	  //	  smc_open_batch(a[0], checksum, verifySize, threadID);
	  for(int i = 0; i < size; i++){
	    int checksum = smc_open(result[0][i], threadID);
	    //printf("%d ", checksum[i]);
	    printf("result[%d] %d \n", i, checksum);
	      //return;
	  }
...
void SMC_Utils::smc_leq_mal(mpz_t* a, mpz_t* b, mpz_t* result, int alen, int blen, int resultlen, std::string type, int threadID){
  mpz_t* sub = (mpz_t*)malloc(sizeof(mpz_t) * 2);
  for(int i=0; i<2; ++i){
    mpz_init(sub[i]);
  }
...
void SMC_Utils::smc_priv_eval_mal(mpz_t* a, mpz_t* b, mpz_t* cond, int threadID){
	 ...
	 //         Mul->doOperation_mal(results, op1, op2, 1, threadID);
...
void SMC_Utils::smc_open_batch(mpz_t* var, int* result, int size, int threadID){
        mpz_t* data = (mpz_t*)malloc(sizeof(mpz_t) * size);
        ...
        nNet.broadcastToPeers(data, size, buffer, threadID);
        ss->reconstructSecret(results, buffer, size, false);
        mpz_t tmp, field;
        mpz_init(tmp);
        mpz_init(field);
        ss->getFieldSize(field);
        for(int i = 0; i < size; i++){
          mpz_mul_ui(tmp, results[i], 2);
          //gmp_printf ("temp at %d is  %Zd\n", i, tmp);
          if(mpz_cmp(tmp, field) > 0)
            mpz_sub(results[i], results[i], field);
          //gmp_printf("%Zd ", results[0]);
          result[i] = mpz_get_si(results[i]);
        }
        //free the memory
        for(int i = 0; i < size; i++){
          mpz_clear(data[i]);
